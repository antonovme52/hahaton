...
function removeFinishedThreads(state: ScratchVmState) {
  const threads = state.threads.filter((thread) => !thread.done && thread.stack.length > 0);
  return {
    ...state,
    threads,
...
function executeBlock(state: ScratchVmState, threadIndex: number, block: WorkspaceBlock): ScratchVmState {
  ...
  switch (block.id) {
    ...
    case "sensing-ask-and-wait":
      nextState = appendLog(nextState, "error", "Блок `спросить и ждать` пока не поддерживает пользовательский ввод в VM.");
      break;
...
function executeThreadItem(state: ScratchVmState, index: number): ScratchVmState {
  ...
  let thread: VmThread = {
    ...sourceThread,
    sleepUntil: sourceThread.sleepUntil !== null && sourceThread.sleepUntil <= state.timeMs ? null : sourceThread.sleepUntil,
    waitingCondition:
      sourceThread.waitingCondition && evaluateCondition(sourceThread.waitingCondition, sourceThread.spriteId, state)
        ? null
        : sourceThread.waitingCondition
  };
...
export function advanceVmFrame(state: ScratchVmState, deltaMs: number) {
  ...
  let nextState: ScratchVmState = clearExpiredBubbles({
    ...state,
