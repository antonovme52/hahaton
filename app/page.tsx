  ...
  return (
    <div className="mx-auto max-w-[1680px] space-y-10 px-4 py-8 sm:px-6 lg:px-8 2xl:max-w-[1760px] 2xl:space-y-12 2xl:px-10 2xl:py-10">
      ...
      <section className="grid gap-6 xl:grid-cols-[0.94fr_1.06fr] 2xl:gap-8">
        <Card className="overflow-hidden border-2 border-white/25 bg-pop-ink text-white shadow-[0_26px_70px_rgba(36,48,79,0.18)] ring-1 ring-white/15">
          <CardContent className="relative space-y-6 p-6 sm:p-8">
            ...
            <div className="relative">
              <p className="inline-flex items-center gap-2 rounded-full border border-white/12 bg-white/8 px-4 py-2 text-base font-semibold text-pop-sun">
                <Sparkles className="h-4 w-4" />
                Как устроено обучение
              </p>
              <h2 className="mt-5 text-3xl font-black sm:text-4xl">Меньше текста, больше движения вперед</h2>
            ...
            <div className="relative grid gap-4">
              {journey.map((item) => (
                <div key={item.step} className="rounded-[28px] border border-white/22 bg-white/8 p-5 backdrop-blur">
                  <div className="flex items-start gap-4">
                    <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-white text-pop-ink">
                      <span className="text-base font-black">{item.step}</span>
                    </div>
                    <div>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {benefits.map((item) => {
            ...
            return (
              <Card key={item.title} className={`border-2 border-border bg-gradient-to-br ${item.className} shadow-[0_22px_50px_rgba(47,71,104,0.08)] ring-1 ring-pop-ink/8`}>
                <CardContent className="space-y-4 p-6 2xl:p-7">
                  ...
                  <div>
                    <h2 className="text-xl font-semibold text-pop-ink">{item.title}</h2>
      ...
      <section className="rounded-[40px] border-2 border-border bg-gradient-to-br from-white via-[#f4f8ff] to-[#f6f2ff] px-6 py-8 shadow-[0_22px_55px_rgba(47,71,104,0.08)] ring-1 ring-pop-ink/8 sm:px-8 sm:py-10 2xl:px-12">
        <div className="grid gap-8 lg:grid-cols-[1fr_auto] lg:items-end">
          <div>
            <p className="text-base font-semibold uppercase tracking-[0.28em] text-pop-coral">Готово к запуску</p>
            <h2 className="mt-3 max-w-3xl text-3xl font-black text-pop-ink sm:text-4xl lg:text-5xl">
