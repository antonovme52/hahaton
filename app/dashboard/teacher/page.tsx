      ...
      <div className="space-y-8">
        <section className="space-y-2">
          ...
          <p className="max-w-3xl text-muted-foreground">
            Управляй группами, конструируй задания и следи, как ученики проходят материалы и coding-челленджи.
          </p>
        </section>

        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <StatCard label="Заданий" value={String(data.summary.totalAssignments)} helper="В конструкторе и аналитике" icon={BookOpen} />
          <StatCard label="Групп" value={String(data.summary.activeGroups)} helper="Teacher-owned группы" icon={FolderKanban} tone="from-pop-sky to-cyan-500" />
          <StatCard label="Отправок" value={String(data.summary.submissionCount)} helper="Все student-попытки" icon={Activity} tone="from-pop-plum to-fuchsia-500" />
          <StatCard label="Точность" value={`${data.summary.averageCorrectness}%`} helper="Средний correct rate" icon={Users} tone="from-pop-sun to-pop-coral" />
        </div>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground">Быстрый обзор статуса и эффективности.</p>
              </div>
            ...
            <CardContent className="space-y-4">
              ...
              {!data.assignments.length ? (
                <p className="text-sm text-muted-foreground">Пока нет заданий. Создай первое в конструкторе.</p>
              ) : null}
            </CardContent>
          </Card>
        ...
        <section className="space-y-4">
          <div>
            ...
            <p className="text-muted-foreground">Группы используются для назначения заданий и фильтра рейтинга.</p>
          </div>
                ...
                <CardContent className="space-y-4 p-6">
                  ...
                  <div className="space-y-2">
                    ...
                    {!group.members.length ? (
                      <p className="text-sm text-muted-foreground">Пока нет прикреплённых учеников.</p>
                    ) : null}
                  </div>
                </CardContent>
              </Card>
